      ...
      try {
        ...
        if (searchResult.length > 0) {
            ...
            var archiveFolderId = search
              .create({
                type: search.Type.FOLDER,
                filters: [['name', 'is', 'Archive Customer CSV']],
                columns: ['internalid']
              })
              .run()
              .getRange({ start: 0, end: 1 })
            ...
            if (archiveFolderId == null) {
                ...
                folder.setValue({ fieldId: 'name',
                        value: 'Archive Customer CSV' });
                        archiveFolderId = folder.save();
                log.debug("Made Archive Customer CSV folder in NetSuite File Cabinet with Id ! " + archiveFolderId);
            }
            ...
            */
            ...
            for (var i = 0; i < searchResult.length; i++) {
              try {
                ...
                log.debug("Customer CSV File Uploaded Successfully to SFTP server with file Id " + fileObj.id);
                ...
                log.error({
                  title: 'Error in uploading customer csv files',
                  details: e,
                });
              }
            }
        ...
        log.error({
          title: 'Error processing in customer csv files',
          details: e,
        });
        throw error.create({
          name:"Error processing in customer csv files",
          message: e
        });
      }
    }
