...
define(['N/file', 'N/search', 'N/sftp', 'N/error'],
    (file, search, sftp, error) => {
        const getInputData = (inputContext) => {
            ...
            var discountItemSearch = search.load({ id: 'customsearch_hc_export_discount_items' });
        ...
        const summarize = (summaryContext) => {
            try {
                ...
                if (totalRecordsExported > 0) {
                    var fileName =  summaryContext.dateCreated + '-DiscountItemExport.csv';
                    ...
                    */
                    ...
                    log.debug("Discount Item CSV File Uploaded Successfully to SFTP server with file" + fileName);
                }
            } catch (e) {
                log.error({
                title: 'Error in exporting and uploading discount item csv files',
                details: e,
                });
                throw error.create({
                name:"Error in exporting and uploading discount item csv files",
                message: e
                });
            }            
        }
