      ...
      try {
        ...
        var folderId = search
          .create({
            ...
            filters: [['name', 'is', 'Export ALC Audit Log Report CSV']],
            columns: ['internalid']
          })
          .run()
        ...
        if (searchResult.length > 0) {
            ...
            // Check Archive Export ALC Audit Log Report CSV Folder is created or not 
            var archiveFolderId = search
              .create({
                type: search.Type.FOLDER,
                filters: [['name', 'is', 'Archive Export ALC Audit Log Report CSV']],
                columns: ['internalid']
              })
              .run()
              .getRange({ start: 0, end: 1 })
            ...
            // Made Archive Export ALC Audit Log Report CSV folder in NetSuite File Cabinet
            if (archiveFolderId == null) {
                var folder = record.create({ type: record.Type.FOLDER});
                folder.setValue({ fieldId: 'name',
                        value: 'Archive Export ALC Audit Log Report CSV' });
                        archiveFolderId = folder.save();
                log.debug("Made Archive Export ALC Audit Log Report CSV folder in NetSuite File Cabinet with Id ! " + archiveFolderId);
            }
            ...
            */
            ...
            for (var i = 0; i < searchResult.length; i++) {
              try {
                ...
                log.debug("Export ALC Audit Log Report CSV File Uploaded Successfully to SFTP server with file Id " + fileObj.id);
                ...
                log.error({
                  title: 'Error in uploading Export ALC Audit Log Report CSV files',
                  details: e,
                });
              }
            }
        ...
        log.error({
          title: 'Error in processingExport ALC Audit Log Report CSV files',
          details: e,
        });
        throw error.create({
          name:"Error in processing Export ALC Audit Log Report CSV files",
          message: e
        });
      }
    }
