    ...
    function execute(context) {
      try {
        ...
        if (searchResult.length > 0) {
            ...
            // Check Archive Fulfilled Transfer Order CSV Folder is created or not 
            var archiveFolderId = search
              .create({
                type: search.Type.FOLDER,
                filters: [['name', 'is', 'Archive Fulfilled Transfer Order CSV']],
                columns: ['internalid']
              })
              .run()
              .getRange({ start: 0, end: 1 })
            ...
            if (archiveFolderId == null) {
                ...
                folder.setValue({ fieldId: 'name',
                        value: 'Archive Fulfilled Transfer Order CSV' });
                        archiveFolderId = folder.save();
                log.debug("Made Archive Fulfilled Transfer Order CSV folder in NetSuite File Cabinet with Id ! " + archiveFolderId);
            }
            ...
            */
            ...
            for (var i = 0; i < searchResult.length; i++) {
              try {
                ...
                log.debug("Fulfilled Transfer Order CSV File Uploaded Successfully to SFTP server with file Id " + fileObj.id);
                ...
                log.error({
                  title: 'Error in uploading fulfilled transfer order csv files',
                  details: e,
                });
              }
            }
        ...
        log.error({
          title: 'Error processing in fulfilled transfer order csv files',
          details: e,
        });
        throw error.create({
          name:"Error processing in fulfilled transfer order csv files",
          message: e
        });
      }
    }
