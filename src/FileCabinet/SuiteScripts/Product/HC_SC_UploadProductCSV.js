    ...
    function execute(context) {
      try {
        ...
        if (searchResult.length > 0) {
            ...
            // Check Archive Export Product CSV Folder is created or not 
            var archiveFolderId = search
              .create({
                type: search.Type.FOLDER,
                filters: [['name', 'is', 'Archive Export Product CSV']],
                columns: ['internalid']
              })
              .run()
              .getRange({ start: 0, end: 1 })
            ...
            // Made Archive Export Product CSV folder in NetSuite File Cabinet
            if (archiveFolderId == null) {
                var folder = record.create({ type: record.Type.FOLDER});
                folder.setValue({ fieldId: 'name',
                        value: 'Archive Export Product CSV' });
                        archiveFolderId = folder.save();
                log.debug("Made Archive Export Product CSV folder in NetSuite File Cabinet with Id ! " + archiveFolderId);
            }
            ...
            */
            ...
            for (var i = 0; i < searchResult.length; i++) {
              try {
                ...
                log.debug("Export Product CSV File Uploaded Successfully to SFTP server with file Id " + fileObj.id);
                ...
                log.error({
                  title: 'Error in uploading export product csv files',
                  details: e,
                });
              }
            }
        ...
        log.error({
          title: 'Error in processing export product csv files',
          details: e,
        });
        throw error.create({
          name:"Error in processing export product csv files",
          message: e
        });
      }
    }
