...
define(['N/file', 'N/record', 'N/search', 'N/sftp', 'N/task', 'N/error'],
    (file, record, search, sftp, task, error) => {
        ...
        const map = (mapContext) => {
            ...
            var externalId = contextValues.values["externalid.createdFrom"].value;
            ...
            var shippedDate = contextValues.values.formulatext;
            ...
            var shipmentData = {
                'orderId': orderId ? orderId : externalId,
        ...
        const reduce = (reduceContext) => {
            ...
            var fulfilledSalesOrderItem = {
                orderMap: orderMap,
            }
            reduceContext.write({
                key: reduceContext.key,
                value: JSON.stringify(fulfilledSalesOrderItem)
            });
        }
        ...
        const summarize = (summaryContext) => {
            try {
                ...
                if (totalRecordsExported > 0) {
                    var fileName = 'SalesOrderFulfillment-' + summaryContext.dateCreated.toISOString().replace(/[:T]/g, '-').replace(/\..+/, '') + '.json';
