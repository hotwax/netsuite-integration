    ...
    function execute(context) {
      try {
        ...
        if (searchResult.length > 0) {
            ...
            // Check Archive Sales Order CSV Folder is created or not 
            var archiveFolderId = search
              .create({
                type: search.Type.FOLDER,
                filters: [['name', 'is', 'Archive Sales Order CSV']],
                columns: ['internalid']
              })
              .run()
              .getRange({ start: 0, end: 1 })
            ...
            // Made Archive Sales Order Order CSV folder in NetSuite File Cabinet
            if (archiveFolderId == null) {
                var folder = record.create({ type: record.Type.FOLDER});
                folder.setValue({ fieldId: 'name',
                        value: 'Archive Sales Order CSV' });
                        archiveFolderId = folder.save();
                log.debug("Made Archive Sales Order CSV folder in NetSuite File Cabinet with Id ! " + archiveFolderId);
            }
            ...
            */
            ...
            for (var i = 0; i < searchResult.length; i++) {
              try {
                ...
                log.debug("Sales Order CSV File Uploaded Successfully to SFTP server with file Id " + fileObj.id);
                ...
                log.error({
                  title: 'Error in uploading sales order csv files',
                  details: e,
                });
              }
            }
        ...
        log.error({
          title: 'Error processing in sales order csv files',
          details: e,
        });
        throw error.create({
          name:"Error processing in sales order csv files",
          message: e
        });
      }
    }
