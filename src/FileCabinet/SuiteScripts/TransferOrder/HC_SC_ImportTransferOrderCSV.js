...
define(['N/sftp', 'N/task', 'N/error', 'N/search', 'N/file', 'N/runtime'], function (sftp, task, error, search, file, runtime) {
    function execute(context) {
        try {
                ...
                if (!list[i].directory) {
                    ...
                    try {
                        ...
                        if (downloadedFile && downloadedFile.size > 0) {
                            ...
                            scriptTask.mappingId = 'custimport_add_transferorders_hc';
                            scriptTask.importFile = downloadedFile;
                            scriptTask.name = 'Add Transfer Order' + '-' + fileName;           
                        ...
                        log.error({
                            title: 'Error in processing transfer order csv files',
                            details: e,
                        });
                        ...
                        if (errorList.length !== 0) {
                            ...
                            var date = new Date();
                            var errorFileName = date + '-ErrorTransferOrder.csv';
            ...
            log.error({
                title: 'Error in importing transfer order csv files',
                details: e,
            });
            throw error.create({
                name:"Error in importing transfer order csv files",
                message: e
            });
        }
   }
